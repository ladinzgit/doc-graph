...
DocGraph AWS 인프라 (Terraform). AWS Learner Lab 환경에서 백엔드 컨테이너 실행까지 검증 완료.
...
| 2단계 | ECR | ✅ |
| 3단계 | ALB + ECS Fargate | ✅ |
| 4단계 | RDS PostgreSQL 15 | ✅ |
| 5단계 | S3 + CloudFront (프론트엔드) | 🔜 미구현 |

Spring Boot 백엔드 컨테이너가 ECS Fargate에서 정상 기동되며 Swagger UI 접속을 확인했습니다.  
접속 주소: `http://<alb-dns>/api/swagger-ui/index.html`
...
```
              ...
              ┌───────────────▼───────────────┐
              │        ALB  (main-alb)         │ ← sg-alb
              │   HTTP :80  →  Target :8080    │   inbound  0.0.0.0/0 :80, :443
              └───────────────┬───────────────┘   outbound  all
                              │
╔═════════════════════════════╪══════════════════════════════════╗
...
║  │         │   1 vCPU / 2 GB  · port 8080       │ ← sg-fargate ║
║  │         │   assign_public_ip = true          │   inbound  sg-alb :8080 ║
║  │         └────────────────┬──────────────────┘   outbound  all ║
║  └─────────────────────────┼─────────────────────────────┘    ║
║                             │ :5432                             ║
║  ┌── Private Subnet ────────┼─────────────────────────────┐    ║
...
║  │         ┌────────────────▼──────────────────┐         │    ║
║  │         │     RDS PostgreSQL 15              │         │    ║
║  │         │   db.t3.micro · 20 GB              │ ← sg-rds ║
║  │         │   publicly_accessible = false      │   inbound  sg-fargate :5432 ║
║  │         └───────────────────────────────────┘   outbound  all ║
║  └─────────────────────────────────────────────────────────┘    ║
║                                                                  ║
...
╚══════════════════════════════════════════════════════════════════╝
              │
    ECR  (public endpoint)
    Fargate가 공인 IP로 직접 pull — NAT Gateway 불필요
```
...
| sg-fargate | sg-alb → :8080 | all (ECR pull · 외부 API) |
...
| **Container** | `MANAGEMENT_SERVER_PORT=8080` · `MaxRAMPercentage=75%` · startPeriod 120s |
| **RDS** | PostgreSQL 15 · db.t3.micro · 20 GB · Multi-AZ off · private subnet |
...
`terraform.tfvars`는 `.gitignore`에 포함되어 있어 커밋되지 않습니다.
...
Redirect URI에는 아래 주소를 등록합니다 (`terraform apply` 후 ALB DNS 확인):

```
http://<alb-dns>/api/login/oauth2/code/notion
```
...
│   ├── rds/     RDS PostgreSQL 15 (db.t3.micro, 프라이빗 서브넷)
│   └── s3_cf/   S3 + CloudFront — 프론트엔드 호스팅 (미사용)
├── scripts/
...
- **퍼블릭 서브넷 + 공인 IP**: NAT Gateway 없이 ECR 이미지 pull. Learner Lab 크레딧 절약.
- **ECS 환경변수로 시크릿 주입**: Secrets Manager 대신 `terraform.tfvars` → ECS 환경변수. Learner Lab IAM 제약 회피.
...
- [ ] React 프론트엔드 구현 후 `module.s3_cf` 활성화 (S3 + CloudFront 배포)
