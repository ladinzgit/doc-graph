...
Webhook 수신은 동기 트랜잭션에서 `WebhookEvent(pending)` 적재 + 200 OK까지만 처리한다. 본문 조회·갱신·엣지 재평가·검증 대상 쌍 전달은 비동기 worker가 이어 받는다.
...
sequenceDiagram
    ...
    alt Webhook 수신 (동기)
        ...
        document->>document: HMAC 검증 + 봇 actor 필터<br/>WebhookEvent(pending) 적재
        document-->>Notion: 200 OK
    end

    Note over document: 비동기 worker가 pending WebhookEvent 또는<br/>초기 동기화 트리거를 수신
    alt Webhook
    ...
    validation->>validation: ValidationRun(pending) 영속화
```

### 정합성 검사

엣지가 생기거나 연결된 문서 내용이 바뀌면 `graph`가 `validation`으로 검증 대상 쌍을 전달한다. 변경 감지 완료, 연결 제안 수락, 수동 엣지 추가 모두 동일한 경로다. 한 검증 대상 쌍 = `ValidationRun` 1건 = AI 호출 1건이며, 실패 격리 단위가 된다.
...
sequenceDiagram
    ...
    Note over validation: 비동기 worker가 pending ValidationRun 1건 수신<br/>두 문서의 본문 스냅샷·블록 row 조회
    validation->>AI: 변경 블록 + 반대편 문서 전체 블록 + 체크 항목
...
- WebhookEvent (수신한 webhook 이벤트, 상태: `pending` / `processing` / `done` / `failed`) — 동기 트랜잭션과 비동기 변경 감지 사이의 outbox

**주요 흐름 — Webhook 수신 (동기)**
1. HMAC 서명 검증
2. Webhook actor 확인 — DocGraph 봇 actor이면 재처리 없이 종료
3. `WebhookEvent(pending)` 적재 후 200 OK 응답

**주요 흐름 — 내용 변경 (비동기 worker)**
1. `WebhookEvent(pending)` 1건 수신
...
- worker 실패 시 `WebhookEvent`는 `pending` 또는 `failed`로 남아 retry · stale row 재처리 worker가 재시도
...
**핵심 개념**
- ValidationRun (검증 실행 이력, 상태: `pending` / `success` / `failed`) — 한 엣지 × 한 변경 batch = ValidationRun 1건 = AI 호출 1건. 동시에 graph → validation 사이의 outbox 역할도 수행한다.
- Conflict (열린 위반. `ignoredAt`, `ignoredBy`, `ignoreReason?` 필드를 가질 수 있음)
- ConflictResult (충돌 구간, 원인, 수정 제안 — ValidationRun에 귀속). 충돌 한 묶음은 블록 N:M 관계: `source_block_ids[]`, `target_block_ids[]`, `rationale`.
...
**주요 흐름**
1. `graph`로부터 검증 대상 쌍 수신 (변경 감지, 제안 수락, 수동 엣지 추가 경로 모두 동일) → `ValidationRun(pending)` 영속화
2. 비동기 worker가 `ValidationRun(pending)` 1건 수신
3. 두 문서의 본문 스냅샷·블록 row 조회, 변경 batch의 변경 블록 식별
4. AI API non-blocking 호출 (타임아웃 30초). 입력 = (변경 블록, 반대편 문서 전체 블록, 체크 항목). 응답 = 블록 N:M 충돌 묶음 배열.
5. ValidationRun 결과 저장, `graph`의 엣지 상태 업데이트
...
- worker 실패 시 `ValidationRun`은 `pending` 또는 `failed`로 남아 retry · stale row 재처리 worker가 재시도
